# 系统优化脚本支持Rocky、Debian、Ubuntu和CentOS



在一些企业大规模部署时，通常需要做些预处理的操作。使用脚本可以大大提高工作效率同时保证品控。本人根据多年工作经验总结如下系统优化配置脚本，各位也可以根据自己的需要修改或增加其他功能/配置。


仓库地址：https://github.com/opstrip/bash-shell
参考了：https://github.com/cby-chen/OS

已针对 `Debian`/`RockyLinux`/`Ubuntu`/`CentOS` 等操作系统优化调试，后续可能会继续更新脚本

shell说明如下：
## 用法
```shell
git clone https://github.com/opstrip/bash-shell
cd bash-shell
sh os.sh [你的主机名]
```
如果未指定参数，默认主机名为 `ip-192-168-101-132` 格式。
...
- 欢迎界面系统信息展示
- `SSH`服务优化配置
- 添加`JMS`跳板机信任[根据实际情况添加]
- 添加主机监控项

## 其他
脚本`os.sh`需要交互，在一些不需要交互的时候如云主机初始化时，可以贴入以init开头的对应脚本，如`initRocky9.sh`。
